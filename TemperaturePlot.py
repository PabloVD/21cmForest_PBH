...
# CDM
ThermalHisF = 'ThermalHistory_Mpbh_{:.1e}_fpbh_{:.1e}_Xi_30_Tmin_1.000e+04_Rfmp_15_chiUV_1.00e+56_Nalpha_4.00e+03.dat'.format(1., 1.e-5)
file = np.loadtxt("21cmFiles/"+ThermalHisF,unpack=True)
ax_T.plot(file[0],Tk_ad(file[0]),color=colcdm,alpha=1,lw=1.)
ax_T.plot(file[0],file[2],color=colcdm,lw=4.,alpha=0.5)#,linewidth=widths[MM],alpha=0.5)
...
for MM, MassPBH in enumerate(MassPBH_vals):
    ...
    for ff, fpbh in enumerate(fpbh_vals):
        ...
        file = np.loadtxt("21cmFiles/"+ThermalHisF,unpack=True)
        ax_T.plot(file[0],file[2],color=cols[ff],linestyle=lins[MM],lw=1.)
        ...
        file = np.loadtxt("21cmFiles/"+ThermalHisF,unpack=True)
        ax_T.plot(file[0],file[2],color=cols[ff],linestyle=lins[MM],lw=4.,alpha=0.5)
